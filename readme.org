#+title: deltaclaw
#+description: Discord agent interface for Even G2 glasses.

Turn a private Discord server into a voice-controlled AI agent hub on your [[https://www.evenrealities.com/][EvenRealities G2]] glasses.

#+begin_example
  scroll channels → tap → speak → agent receives instructions
#+end_example
...
[[file:docs/dc-channels-home-server.png]]

[[file:docs/dc-home-server.png]]

[[file:docs/dc-channels-second-brain.png]]
...
[[https://openclaw.com][OpenClaw]] connects to Discord natively. Tell it to set up in your server and you have a multi-agent operating system in minutes ([[https://youtu.be/vxpuLIA17q4][Alex Finn's walkthrough]]). Each channel is a separate agent with its own history and pinned artifacts. Coding agents, stock research, daily digests, competitor analysis - all running 24/7, chaining outputs across channels.

deltaclaw puts all of that on your glasses:

- *Multichannel* : scroll between agents, each channel is a different agent/context
...
- *Hey Even* : hands-free voice commands route to your main channel without opening the app
...
└──────────────────────┬───────────────────┘
       ...
       ▼               ▼               ▼
┌──────────────┐ ┌──────────────┐ ┌──────────────┐
│  Discord API │ │ stt-anywhere │ │ Hey Even     │
│  (channels,  │ │ (optional)   │ │ (Vercel fn)  │
│   messages)  │ │              │ │              │
└──────────────┘ │ your GPU,    │ │ bridges Even │
                 │ ~500ms,      │ │ AI to your   │
                 │ free, local  │ │ Discord      │
                 └──────────────┘ └──────────────┘
#+end_example
...
Or say "Hey Even" for hands-free access to your main channel without opening the app.
...
The glasses app talks to the Discord API with a bot token configured in settings. For speech-to-text it connects to [[https://github.com/mwlaboratories/stt-anywhere][stt-anywhere]] over WebSocket, or a cloud STT provider. "Hey Even" voice commands go through a Vercel serverless function that bridges Even's native AI to your Discord.
...
- Vercel serverless function for Hey Even
